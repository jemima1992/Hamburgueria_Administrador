...
public class DaoPedido {
   ...
   PedidoBeans  mod = new PedidoBeans ();
    ...
    public void Salvar(PedidoBeans mod){
        ...
        try {
            PreparedStatement pst = ConexãoBD.con.prepareStatement("INSERT into pedidos (Codigo_Pedido,Data, Hora, Valortotal,Status,Mesa,Obeservação)Values(?,?,?,?,?,?,?)");  
    ...
    public void Editar (PedidoBeans mod){
        conex.conexao();
        try {
             PreparedStatement pst = conex.con.prepareStatement("update pedidos set Data = ?, Hora=?, Valortotal=? ,Status= ?, Mesa = ? , Obeservação =? where Codigo_Pedido = ? ");
    ...
    public PedidoBeans buscaPedidos(PedidoBeans mod) {
        ...
        try {
            if (conex.rs.first()) {
                ...
                mod.setMesa(conex.rs.getString(" Obeservação"));
